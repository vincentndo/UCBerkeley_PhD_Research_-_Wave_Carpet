...
c      REAL      Y1(NDX,NDY,NDP),Y2(NDX,NDY,NDP),BAMP(NDX,NDY)
      ...
      COMMON/ABLOCK/DELT,TLEN,TWID,PERIOD,ENERGY0,DEPT,ZETA,GAMMA,MU 
      COMMON/IBLOCK/NUMB 
...
c      CALL INIT(NDX,NDY,NDP,SF,VP,BAMP,FQ,ISTP,VX,VY,WRKX,WRKY)
      CALL INIT(NDX,NDY,NDP,SF,VP,BAMP,VPB,FQ,ISTP,VX,VY,WRKX,WRKY)
      WRITE(*,*) (REAL(BAMP(I,1)),I=1,3)
...
c      WRITE(*,*) "MILESTONE"
      ...
      DO 107 J=1,NUMB(8,2)
      WRITE(15,*) (REAL(SF_temp(I,J)),I=1,NUMB(8,1))
107   WRITE(15,*) (REAL(BAMP_temp(I,J)),I=1,NUMB(8,1))    
C
      RETURN
      END
C****************************
...
c      SUBROUTINE INIT(NDX,NDY,NDP,SF,VP,BAMP,FQ,ISTP,VX,VY,WRKX,WRKY)
      SUBROUTINE INIT(NDX,NDY,NDP,SF,VP,BAMP,VPB,FQ,ISTP,
     1                 VX,VY,WRKX,WRKY)
C
C INITIALIZE DATA
C
...
C
      ...
      DO 250 I=1,NUMB(4,1)
      RB1(I,J)= WB(I,J)
250    RB2(I,J)=(1.0/GAMMA-1)*BAMP(I,J)+ZETA*RB1(I,J)*SQRT(DEPT)
      DO 260 I=NUMB(7,1),NUMB(8,1)
      IM=I-NUMB(10,1)
      RB1(IM,J)= WB(I,J)
260    RB2(IM,J)=(1.0/GAMMA-1)*BAMP(I,J)+ZETA*RB1(IM,J)*SQRT(DEPT)
